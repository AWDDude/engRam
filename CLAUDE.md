...
| `retrieve` | memory_id | — |
| `delete` | memory_id | — |
| `update` | memory_id, plus at least one of: title, content, tags, add_tags, remove_tags, linked_ids | — |
...
`update`'s `title` and `content` are each independently optional — omit either to leave it unchanged. Tags can be changed two ways: `tags` replaces the whole set, or `add_tags`/`remove_tags` change it incrementally (union, then subtract, so a tag in both ends up removed) without touching tags not mentioned. `tags` is rejected if combined with `add_tags`/`remove_tags` — the two are mutually exclusive ways of expressing the same patch.
