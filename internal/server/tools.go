...
func RegisterTools(s *mcpserver.MCPServer, app *App) {
	...
	s.AddTool(
		mcp.NewTool("update",
			mcp.WithDescription("Update an existing memory. Only the provided fields are changed; at least one of title, content, tags, add_tags, remove_tags, or linked_ids must be given. title and content are independently optional — provide only what you want to change."),
			mcp.WithString("memory_id",
				mcp.Required(),
