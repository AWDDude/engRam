...
func RegisterTools(s *mcpserver.MCPServer, app *App) {
	s.AddTool(
		mcp.NewTool("store",
			...
			mcp.WithArray("linked_ids",
				mcp.Description("Optional IDs of other memories to link to (linking is bidirectional). Every ID must name an existing memory: if any does not, the whole call is rejected and no memory is created, so the call can simply be corrected and retried."),
				mcp.WithStringItems(),
			),
		),
	...
	s.AddTool(
		mcp.NewTool("update",
			mcp.WithDescription("Update an existing memory. Only the provided fields are changed; at least one of title, content, tags, add_tags, remove_tags, linked_ids, add_linked_ids, or remove_linked_ids must be given. title and content are independently optional — provide only what you want to change."),
			mcp.WithString("memory_id",
				mcp.Required(),
			...
			mcp.WithArray("tags",
				mcp.Description("New tags (replaces the existing set). Cannot be combined with add_tags or remove_tags. Stored lowercase regardless of the case given."),
				mcp.WithStringItems(),
			),
			mcp.WithArray("add_tags",
				mcp.Description("Tags to add to the existing set, leaving tags not listed untouched. Cannot be combined with tags. Stored lowercase regardless of the case given."),
				mcp.WithStringItems(),
			),
			mcp.WithArray("remove_tags",
				mcp.Description("Tags to remove from the existing set, leaving the rest untouched. Cannot be combined with tags. A tag listed in both add_tags and remove_tags ends up removed."),
				mcp.WithStringItems(),
			),
			mcp.WithArray("linked_ids",
				mcp.Description("New linked memory IDs (replaces the existing set; linking is bidirectional). Cannot be combined with add_linked_ids or remove_linked_ids."),
				mcp.WithStringItems(),
			),
			mcp.WithArray("add_linked_ids",
				mcp.Description("Memory IDs to link, leaving existing links untouched (linking is bidirectional). Cannot be combined with linked_ids. Every ID must name an existing memory: if any does not, the whole update is rejected and the memory is left unchanged."),
				mcp.WithStringItems(),
			),
			mcp.WithArray("remove_linked_ids",
				mcp.Description("Memory IDs to unlink, leaving the rest untouched (unlinking is bidirectional). Cannot be combined with linked_ids. An ID listed in both add_linked_ids and remove_linked_ids ends up unlinked."),
				mcp.WithStringItems(),
			),
		),
